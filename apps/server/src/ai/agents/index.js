...
import { PlacementAnalyticsService } from '../../services/placement-analytics.service.js';
...
const generateAgentCompletion = async (agentName, studentId, snapshot, customParams) => {
  ...
  `;
    return AIService.generateCompletion(studentId, agentName, system, userPrompt);
};
// 1. Student Success Agent
export class StudentSuccessAgent {
    constructor() {
        this.metadata = {
...
export class PlacementReadinessAgent {
    constructor() {
        this.metadata = {
...
export class FacultyInsightsAgent {
    constructor() {
        this.metadata = {
...
export class PlacementAnalyticsAgent {
    constructor() {
        this.metadata = {
    ...
    async execute(snapshot, customParams) {
    ...
    `;
        return AIService.generateCompletion(snapshot.studentId, this.metadata.name, system, userPrompt);
    }
}
// 6. Career Recommendation Agent
export class CareerRecommendationAgent {
    constructor() {
        this.metadata = {
...
export class ResumeIntelligenceAgent {
     ...
     */
    runDeterministicAtsChecks(resumeText) {
    ...
    async execute(snapshot, customParams) {
        ...
        const rawResume = customParams?.resumeContent || '';
        ...
        const verifiedSkills = (snapshot.skills || []).map((s) => s.name || s);
        const verifiedProjects = (snapshot.projects || []).map((p) => p.title || p);
        const userPrompt = `
...
export class JobMatchingAgent {
    ...
    async execute(snapshot, customParams) {
        ...
        const studentSkills = (snapshot.skills || []).map((s) => s.name || String(s));
        const studentData = {
      ...
      - Projects: ${JSON.stringify((snapshot.projects || []).map((p) => p.title || p))}
      - Certifications: ${JSON.stringify((profile?.certifications || []).map((s) => s.name || s))}
...
export class ApplicationStrategyAgent {
    constructor() {
        this.metadata = {
    ...
    async execute(snapshot, customParams) {
        ...
        if (profile?.projects) {
            for (const p of profile.projects) {
                if (p.technologies) {
                    p.technologies.forEach((t) => projectTechs.add(t.toLowerCase().trim()));
                }
            }
        }
    ...
    async execute(snapshot, customParams) {
        ...
        for (const session of interviewSessions) {
            if (session.status === 'COMPLETED') {
                const date = new Date(session.createdAt).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' });
                timelineEvents.push({
                    period: date,
      ...
      - CGPA: ${snapshot.academic?.cgpa ?? 0}
      - Skills: ${(profile?.skills || []).length} skills (${(profile?.skills || []).filter((s) => s.verified).length} verified)
...
export class StudentRiskAgent {
    ...
    async execute(snapshot, customParams) {
      ...
      - Backlogs: ${snapshot.academic?.backlogs ?? 0}
      - Placement Readiness Score: ${profile?.placementReadinessScore ?? 0}/100
      - Skills: ${(profile?.skills || []).map((s) => `${s.name} (${s.proficiency}, ${s.verified ? 'Verified' : 'Declared'})`).join(', ')}
    ...
    `;
        const output = await AIService.generateCompletion(snapshot.studentId, this.metadata.name, system, userPrompt);
        // Add Audit Log
        try {
...
export class StudentEngagementAgent {
    constructor() {
        this.metadata = {
    ...
    async execute(snapshot, customParams) {
      ...
      - Attendance: ${snapshot.academic?.attendance ?? 'N/A'}%
      - Placement Readiness Score: ${profile?.placementReadinessScore ?? 0}/100
      - Skills registered: ${(profile?.skills || []).length} (${(profile?.skills || []).filter((s) => s.verified).length} verified)
...
export class PlacementPreparationAgent {
    constructor() {
        this.metadata = {
    ...
    async execute(snapshot, customParams) {
      ...
      [Job Matching]
      - Eligible job matches: ${JSON.stringify((jobMatching.matches ?? []).filter((m) => m.eligible).map((m) => ({ title: m.jobTitle, company: m.companyName, score: m.matchScore })))}
      ...
      - Learning modules: ${JSON.stringify((learningPath.learningModules ?? []).map((m) => m.moduleTitle))}
...
export class FacultyInterventionAgent {
    constructor() {
        this.metadata = {
...
export class PlacementOfficerCopilot {
    constructor() {
        this.metadata = {
...
export class AdminIntelligenceAgent {
    constructor() {
        this.metadata = {
...
export class DataQualityAgent {
    ...
    async execute(snapshot, customParams) {
        ...
        for (const p of profiles) {
            ...
            const skillNames = (p.skills || []).map((s) => s.name?.toLowerCase()?.trim());
...
export class AIGovernanceAgent {
    constructor() {
        this.metadata = {
            name: 'ai_governance',
            version: '1.0.0',
...
export class FacultyAssistantAgent {
    constructor() {
        this.metadata = {
