...
import { StudentMark } from '../models/StudentMark';
...
const ensureFacultyDataSeeded = async (facultyId: string) => {
...
export const getAssignedClasses = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const facultyId = req.user?.id;
    if (!facultyId) throw new BadRequestError('Faculty ID required');
...
export const getClassDetail = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { classId } = req.params;
    if (!classId) throw new BadRequestError('Class ID parameter required');
...
export const submitAttendance = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const getAttendanceLogs = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!classId) throw new BadRequestError('classId parameter is required');
...
export const submitMarks = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const getStudentMarks = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!assessmentId) throw new BadRequestError('assessmentId is required');
...
export const getAtRiskStudents = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const facultyId = req.user?.id;
    if (!facultyId) throw new BadRequestError('Faculty ID context is required');
    ...
    const formattedRiskList = riskStudents.map(s => {
      const reasons = [];
      if (s.cgpa < 6.0) reasons.push('Cumulative GPA below threshold limits (< 6.0)');
      if (s.placementReadinessScore < 75) reasons.push('Low attendance threshold or placements eligibility warnings');
      ...
      return {
        ...
        name: s.user ? (s.user as any).name : 'Academic Mentee',
        email: s.user ? (s.user as any).email : '',
        reasons,
...
export const getFacultyTimetable = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const facultyId = req.user?.id;
    if (!facultyId) throw new BadRequestError('Faculty context required');
