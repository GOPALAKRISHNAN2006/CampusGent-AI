...
export const updateProfile = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Update fields
    if (validatedData.rollNumber) profile.rollNumber = validatedData.rollNumber;
    ...
    if (validatedData.skills) profile.skills = validatedData.skills as any;
    if (validatedData.projects) profile.projects = validatedData.projects;
    if (validatedData.certifications) profile.certifications = validatedData.certifications as any;
    if (validatedData.githubProfile !== undefined) profile.githubProfile = validatedData.githubProfile;
    if (validatedData.linkedinProfile !== undefined) profile.linkedinProfile = validatedData.linkedinProfile;
    if (validatedData.portfolioUrl !== undefined) profile.portfolioUrl = validatedData.portfolioUrl;
    if (validatedData.careerInterests) profile.careerInterests = validatedData.careerInterests;
    if (validatedData.careerGoals) profile.careerGoals = validatedData.careerGoals;
    ...
    if (validatedData.skills || validatedData.careerGoals || validatedData.careerInterests) {
      await AIInsight.updateMany(
        { user: userId, $or: [{ type: 'LEARNING_PATH' }, { agent: 'learning_path' }] },
        { status: 'STALE' }
      );
    }
...
export const getStudentsList = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
