...
export const finishSession = async (req, res, next) => {
    try {
        ...
        if (profile) {
            profile.placementReadinessScore = Math.round((profile.placementReadinessScore + session.overallScore) / 2);
            await profile.save();
        }
        // 2. Integration: Invalidate existing learning paths and skill gaps
        await AIInsight.updateMany({ user: new mongoose.Types.ObjectId(studentId), agent: 'learning_path' }, { status: 'STALE' });
        await AIInsight.updateMany({ user: new mongoose.Types.ObjectId(studentId), agent: 'skill_gap' }, { status: 'STALE' });
        res.status(200).json({
            success: true,
            data: session,
        });
