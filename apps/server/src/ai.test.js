...
import { PlacementAnalyticsService, clearAnalyticsCache } from './services/placement-analytics.service.js';
...
describe('🤖 Master AI Agents Ecosystem & Observability Suite', () => {
    ...
    it('should reject recommendation queries referencing non-existent data models', () => {
        const studentSnapshot = {
            ...
            skills: [{ name: 'React', proficiency: 'ADVANCED', verified: true }],
            projects: [],
            applications: [],
            interviews: [],
        };
    ...
    describe('🎯 Learning Path Agent Edge Cases', () => {
        ...
        it('should mark existing learning paths as STALE when student skills change', async () => {
            ...
            await AIInsight.updateMany({ user: '654321098765432109876543', $or: [{ type: 'LEARNING_PATH' }, { agent: 'learning_path' }] }, { status: 'STALE' });
    ...
    describe('📄 Resume Intelligence Agent — ATS, Injection & Hallucination Guards', () => {
        ...
        it('should compute deterministic ATS section checks in the agent without AI', () => {
            const agent = AIOrchestrator.getAgent('resume_intelligence');
            expect(agent).toBeDefined();
            const checks = agent.runDeterministicAtsChecks('Education: B.Tech\nSkills: TypeScript\nProjects: Built a dashboard\nSummary: Engineer');
    ...
    describe('💼 Job Matching Agent — Eligibility Boundaries & Hybrid Matching', () => {
        ...
        it('should compute skill overlap deterministically with case-insensitive matching', () => {
            const { matchingSkills, missingSkills, skillMatchPercent } = JobMatchingService.computeSkillMatch(['typescript', 'React', 'NODE.JS'], ['TypeScript', 'React', 'Docker', 'Kubernetes']);
        ...
        it('should return empty matches array when no active jobs exist', async () => {
            const results = await JobMatchingService.findTopMatches({ cgpa: 8.0, backlogs: 0, department: 'dept1', skills: ['TypeScript'] }, 5);
    ...
    describe('🛡️ AI Governance & Telemetry Auditor Agent', () => {
        ...
        it('should automatically flag and log prompt injection attempts during completion requests', async () => {
            ...
            await AIService.generateCompletion('654321098765432109876543', 'career_advisor', 'You are an advisor', injectionPrompt);
            const log = (await AuditLog.findOne({ action: 'AI_PROMPT_INJECTION_ATTEMPT' }).lean());
    ...
    describe('⛓️ AI Orchestrator Layer — Core Routing & Multi-Agent Execution', () => {
        it('should execute multi-agent flow successfully, resolving dependencies and returning synthesis', async () => {
            const output = await AIOrchestrator.executePipeline('654321098765432109876543', ['placement_preparation'], {}, 'ADMIN');
            expect(output).toBeDefined();
        ...
        it('should isolate failures when one agent fails, continuing pipeline with partial results', async () => {
            ...
            const output = await AIOrchestrator.executePipeline('654321098765432109876543', ['student_engagement'], {}, 'ADMIN');
            expect(output).toBeDefined();
        ...
        it('should recover gracefully when an agent times out after 1000ms', async () => {
            ...
            const output = await AIOrchestrator.executePipeline('654321098765432109876543', ['student_engagement'], { timeoutMs: 1000 }, 'ADMIN');
            expect(output).toBeDefined();
            expect(output.student_risk.success).toBe(false);
        ...
        it('should reject unauthorized requests when student runs restricted administrative agents', async () => {
            const output = await AIOrchestrator.executePipeline('654321098765432109876543', ['data_quality'], {}, 'STUDENT');
        ...
        it('should filter out duplicate agent executions to protect model tokens', async () => {
            const output = await AIOrchestrator.executePipeline('654321098765432109876543', ['student_risk', 'student_risk'], {}, 'ADMIN');
        ...
        it('should reject requests with cost-control block if requested pipeline exceeds 10 agents', async () => {
            ...
            await expect(AIOrchestrator.executePipeline('654321098765432109876543', expensivePipeline, { enforceBudget: true }, 'STUDENT')).rejects.toThrow('Cost control block');
        });
    });
});
